...
from hypernet_agentless.client.common import command
...
def _find_resourceid_by_name(client, resource, name):
    ...
    if len(info) > 1:
        raise exceptions.HypernetClientNoUniqueMatch(resource=resource,
                                                   name=name)
    elif len(info) == 0:
...
def find_resourceid_by_name_or_id(client, resource, name_or_id):
    ...
    except exceptions.HypernetClientException:
...
class TableFormater(table.TableFormatter):
    ...
    https://bugs.launchpad.net/python-hypernetclient/+bug/1165962
    """
...
class HypernetCommandMeta(abc.ABCMeta):
    def __new__(cls, name, bases, cls_dict):
        if 'log' not in cls_dict:
            cls_dict['log'] = logging.getLogger(
                cls_dict['__module__'] + '.' + name)
        return super(HypernetCommandMeta, cls).__new__(cls,
                                                     name, bases, cls_dict)


@six.add_metaclass(HypernetCommandMeta)
class HypernetCommand(command.OpenStackCommand):
    ...
    def __init__(self, app, app_args):
        super(HypernetCommand, self).__init__(app, app_args)
        # NOTE(markmcclain): This is no longer supported in cliff version 1.5.2
        # see https://bugs.launchpad.net/python-hypernetclient/+bug/1265926

        # if hasattr(self, 'formatters'):
        # self.formatters['table'] = TableFormater()

    def get_client(self):
        return self.app.client_manager.hypernet

    def get_parser(self, prog_name):
        parser = super(HypernetCommand, self).get_parser(prog_name)
        parser.add_argument(
            '--request-format',
        ...
        if self.resource in data:
            for k, v in six.iteritems(data[self.resource]):
                if isinstance(v, list):
                    ...
                    value = json.dumps(v, indent=self.json_indent)
                    data[self.resource][k] = value
                elif v is None:
...
class CreateCommand(HypernetCommand, show.ShowOne):
    ...
    def get_data(self, parsed_args):
        ...
        obj_creator = getattr(hypernet_client,
...
class UpdateCommand(HypernetCommand):
    ...
    def run(self, parsed_args):
        ...
        if self.allow_names:
            _id = find_resourceid_by_name_or_id(
                hypernet_client, self.resource, parsed_args.id)
        else:
            _id = find_resourceid_by_id(
                hypernet_client, self.resource, parsed_args.id)
        obj_updator = getattr(hypernet_client,
...
class DeleteCommand(HypernetCommand):
    ...
    def run(self, parsed_args):
        ...
        hypernet_client = self.get_client()
        hypernet_client.format = parsed_args.request_format
        obj_deleter = getattr(hypernet_client,
...
class ListCommand(HypernetCommand, lister.Lister):
    ...
    def call_server(self, hypernet_client, search_opts, parsed_args):
        resource_plural = _get_resource_plural(self.resource, hypernet_client)
        obj_lister = getattr(hypernet_client, "list_%s" % resource_plural)
        data = obj_lister(**search_opts)
        return data

    def retrieve_list(self, parsed_args):
        """Retrieve a list of resources from Hypernet server"""
        hypernet_client = self.get_client()
        hypernet_client.format = parsed_args.request_format
        _extra_values = parse_args_to_dict(self.values_specs)
        _merge_args(self, parsed_args, _extra_values,
        ...
        data = self.call_server(hypernet_client, search_opts, parsed_args)
        collection = _get_resource_plural(self.resource, hypernet_client)
        return data.get(collection, [])

    def extend_list(self, data, parsed_args):
        """Update a retrieved list.
        This method provides a way to modify a original list returned from
        the hypernet server. For example, you can add subnet cidr information
        to a list network.
        """
        pass
...
class ShowCommand(HypernetCommand, show.ShowOne):
    ...
    def get_data(self, parsed_args):
        ...
        obj_shower = getattr(hypernet_client, "show_%s" % self.resource)
