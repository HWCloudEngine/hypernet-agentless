...
class OwnerCheck(policy.Check):
    ...
    def __call__(self, target, creds, enforcer):
        if self.target_field not in target:
            ...
            manager = importutils.import_module('hypernet.server.manager')
            f = getattr(manager.HypernetManager.get_instance().plugin,
                        'get_%s' % parent_res)
            # f *must* exist, if not found it is better to let hypernet
            # explode. Check will be performed with admin context
            context = importutils.import_module('hypernet.server.api.context')
            try:
...
def check(context, action, target, plugin=None, might_not_exist=False,
    ...
    :param context: hypernet context
