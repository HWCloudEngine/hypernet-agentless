...
def _get_pagination_max_limit():
    ...
    if (cfg.CONF.pagination_max_limit.lower() !=
            hs_constants.PAGINATION_INFINITE):
        try:
...
def get_sorts(request, attr_info):
    ...
    valid_dirs = [hs_constants.SORT_DIRECTION_ASC,
                  hs_constants.SORT_DIRECTION_DESC]
    ...
    if invalid_dirs:
        msg = (_("%(invalid_dirs)s is invalid value for sort_dirs, "
               ...
               {'invalid_dirs': invalid_dirs,
                'asc': hs_constants.SORT_DIRECTION_ASC,
                'desc': hs_constants.SORT_DIRECTION_DESC})
        raise exc.HTTPBadRequest(explanation=msg)
    return zip(sort_keys,
               [x == hs_constants.SORT_DIRECTION_ASC for x in sort_dirs])
...
class HypernetController(object):
    """Base controller class for Hypernet API."""
    # _resource_name will be redefined in sub concrete controller
    _resource_name = None

    def __init__(self, plugin):
        self._plugin = plugin
        super(HypernetController, self).__init__()
