...
class AWSProvider(provider_api.ProviderDriver):
    ...
    def delete_sgs(self, tenant_id):
        ...
        for sg in resp['SecurityGroups']:
            ...
            if sg['GroupName'] == vm_sg_name:
                vm_sg = sg['GroupId']
        self.ec2_resource.SecurityGroup(hs_sg).revoke_ingress(
                            IpPermissions=[{
                'IpProtocol': '-1',
                'FromPort': 0,
                'ToPort': 65535,
        ...
        self.ec2_resource.SecurityGroup(vm_sg).revoke_ingress(
                            IpPermissions=[{
                'IpProtocol': '-1',
                'FromPort': 0,
                'ToPort': 65535,
        ...
        self.ec2.delete_security_group(GroupId=hs_sg)
        self.ec2.delete_security_group(GroupId=vm_sg)
    ...
    def _aws_instance_to_dict(self, aws_instance):
        ...
        for net_int in aws_instance.network_interfaces_attribute:
            ...
            if net_int['SubnetId'] == self._cfg.data_network():
                ...
                is_vm_net = False
            if net_int['SubnetId'] == self._cfg.fip_network():
                fip_ip = net_int['PrivateIpAddress']
                is_vm_net = False
                if 'Association' in net_int:
                    eip = net_int['Association']['PublicIp']
            if is_vm_net:
                vms_ips.append({
    ...
    def delete_hyperswitch(self, hyperswitch_id):
        ...
        if self.get_eip(hyperswitch_id):
    ...
    def _get_primary_eni(self, hyperswitch_id):
        ...
        aws_instance = self.ec2.describe_instances(
            InstanceIds=[aws_instance_id])['Reservations'][0]['Instances'][0]
    ...
    def associate_eip(self, hyperswitch_id, eip):
        if eip:
            ...
            self.ec2.associate_address(NetworkInterfaceId=primary_eni_id,
                                       AllocationId=eip)

    def disassociate_eip(self, hyperswitch_id):
        addresses = self.ec2.describe_addresses(
            PublicIps=[self.get_eip(hyperswitch_id)])
